...
JavaScript作為一門函數編程語言，將函數看成一種值，與其他數據類型地位相同，因此可以將函數作為返回值或參數傳入其他函數，或是賦值給變量和對象的屬性。
...
函數作用域一節有提到，函數有自己的作用域，在函數外部是讀不到函數作用域內的變量的。函數在執行後就被消滅，其作用域和變量都隨之消失，如果想讀取函數內變量，必須使用閉包，亦即定義在函數內的函數。

閉包的特點，就是可以記住誕生的環境。由此特點，衍生出的兩個閉包的功用是

1. 讀取函數內部變量，且該變量會一直存在記憶體中。
2. 封裝私有屬性和方法。

因為閉包的函數內變量不會被刪除，每次呼叫函數又都會產生新的閉包，所以要小心使用，避免產生效能問題。
...
直接使用時，作用域就是當前作用域；間接調用時，總是使用全局作用域。
